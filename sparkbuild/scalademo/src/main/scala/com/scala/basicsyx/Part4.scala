...
object Part4 extends App {
  ...
  /**
    * return表达式
    *
    *     Scala 函数一般不需要使用return来返回函数值。但是return可以用来从一个匿名
    * 函数中返回值给包含这个匿名函数的带名函数，对于控制抽象来说是很有用的。如果需
    * 要在带名参数中使用return，则需要在定义中给出返回类型
    */
  ...
  printf("abcdefg 中 c 的位置 ：%s%n" ,indexOf("abcdefg",'c'))
